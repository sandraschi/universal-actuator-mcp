...
# PyInstaller spec — produce universal-actuator-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller universal-actuator-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/universal_actuator_mcp/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/universal_actuator_mcp", "universal_actuator_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
...
exe = EXE(
    ...
    a.datas,
    [],
    name="universal-actuator-mcp-backend",
    debug=False,
