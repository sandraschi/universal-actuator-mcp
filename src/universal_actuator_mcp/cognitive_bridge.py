...
class CognitiveBridge:
    ...
    async def get_semantic_context(self, query: str, limit: int = 5) -> str:
        ...
        try:
            ...
            result = await self.fleet.call_tool(
                "knowledge", "adn_knowledge_rag", {"query": query, "limit": limit}
            )
            ...
            logger.warning(
                f"Cognitive Bridge: Context retrieval failed (is 'knowledge' server running?): {e}"
            )
    ...
    async def record_milestone(self, title: str, summary: str, tags: str | None = None):
        ...
        try:
            ...
            actual_tags = (
                tags if tags else "#milestone, #universal-actuator, #empirical-verification"
            )
